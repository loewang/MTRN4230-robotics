...
function [CBData] = ConveyorImageProcess(img,cameraConveyorParams,R,t,UIaxes)
...
%     figure(9);
    ...
    for k = 1 : length(blockprops)
        if blockprops(k).Area > 30000 && blockprops(k).Area < 60000
            ...
            rectangle('Position', [BigBox(1),BigBox(2),BigBox(3),BigBox(4)],'EdgeColor','g','LineWidth',2,'Parent',UIaxes) ;
            blockX(k) = blockprops(k).Centroid(1);%Temp values to store region props in
            blockY(k) = blockprops(k).Centroid(2);
            plot(UIaxes,blockX(k), blockY(k), 'go');
%             hold on;
        end
    end
    ...
    for i = 1:length(OrangeBlocks)
        ...
        rectangle('Position', [BBO(1),BBO(2),BBO(3),BBO(4)],'EdgeColor','m','LineWidth',2,'Parent',UIaxes);
%         hold on;
        plot(UIaxes,CBData(counter,1),CBData(counter,2),'mo'); 
        counter = counter + 1;
    end
    ...
    for i = 1:length(RedBlocks)
        ...
        rectangle('Position', [BBR(1),BBR(2),BBR(3),BBR(4)],'EdgeColor','m','LineWidth',2,'Parent',UIaxes);
%         hold on;
        plot(UIaxes,CBData(counter,1),CBData(counter,2),'mo'); 
        counter = counter + 1;
    end
    ...
    for i = 1:length(YellowBlocks)
        ...
        rectangle('Position', [BBY(1),BBY(2),BBY(3),BBY(4)],'EdgeColor','m','LineWidth',2,'Parent',UIaxes);
%         hold on;
        plot(UIaxes,CBData(counter,1),CBData(counter,2),'mo'); 
        counter = counter + 1;
    end
    ...
    for i = 1:length(GreenBlocks)
        ...
        rectangle('Position', [BBG(1),BBG(2),BBG(3),BBG(4)],'EdgeColor','m','LineWidth',2,'Parent',UIaxes);
%         hold on;
        plot(UIaxes,CBData(counter,1),CBData(counter,2),'mo'); 
        counter = counter + 1;
    end
    ...
    for i = 1:length(BlueBlocks)
        ...
        rectangle('Position', [BBB(1),BBB(2),BBB(3),BBB(4)],'EdgeColor','m','LineWidth',2,'Parent',UIaxes);
%         hold on;
        plot(UIaxes,CBData(counter,1),CBData(counter,2),'mo'); 
        counter = counter + 1;
    end
    ...
    for i = 1:length(PurpleBlocks)
        ...
        rectangle('Position', [BBP(1),BBP(2),BBP(3),BBP(4)],'EdgeColor','m','LineWidth',2,'Parent',UIaxes);
%         hold on;
        plot(UIaxes,CBData(counter,1),CBData(counter,2),'mo'); 
        counter = counter + 1;
    end
    ...
    for k = 1 : length(blockprops2)
        if blockprops2(k).Area > 100 && blockprops2(k).Area < 1000 && blockprops2(k).BoundingBox(3) < 40 && blockprops2(k).BoundingBox(3) > 14 && blockprops2(k).BoundingBox(4) > 14 && blockprops2(k).BoundingBox(4) < 40
            ...
            rectangle('Position', [BBTest(1),BBTest(2),BBTest(3),BBTest(4)],'EdgeColor','r','LineWidth',2,'Parent',UIaxes) ;

            blockX(k) = blockprops2(k).Centroid(1);%Temp values to store region props in
            blockY(k) = blockprops2(k).Centroid(2);
            plot(UIaxes,blockX(k), blockY(k), 'ro');
